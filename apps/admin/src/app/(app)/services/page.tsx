...
export default function ServicesPage() {
  ...
  const services = useApi<Paginated<Service>>(
    `/admin/catalog/services?page=${page}&limit=10&search=${encodeURIComponent(search)}`,
  );
  const categories = useApi<CategoryDoc[]>('/admin/catalog/categories/all');
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editing) {
        await api(`/admin/catalog/services/${editing.id}`, { method: 'PATCH', body: JSON.stringify(payload) });
        toast.success('Service updated.');
      } else {
        await api('/admin/catalog/services', { method: 'POST', body: JSON.stringify(payload) });
