...
export default function EngagementPricingPage() {
  ...
  const bundles = useApi<Paginated<EngagementBundle>>(
    `/admin/engagement-bundles?${query.toString()}`,
  );
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editing) {
        await api(`/admin/engagement-bundles/${editing.id}`, { method: 'PATCH', body: JSON.stringify(payload) });
        toast.success('Bundle updated.');
      } else {
        await api('/admin/engagement-bundles', { method: 'POST', body: JSON.stringify(payload) });
  ...
  async function toggleStatus(bundle: EngagementBundle) {
    ...
    try {
      await api(`/admin/engagement-bundles/${bundle.id}/status`, {
        method: 'PATCH',
  ...
  async function confirmDelete() {
    ...
    setDeleteBusy(true);
    try {
      await api(`/admin/engagement-bundles/${deleting.id}`, { method: 'DELETE' });
