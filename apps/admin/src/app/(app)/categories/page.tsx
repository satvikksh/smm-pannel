...
export default function CategoriesPage() {
  ...
  const categories = useApi<PaginatedCategories>(`/admin/catalog/categories?page=${page}&limit=10`);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editing) {
        await api(`/admin/catalog/categories/${editing._id}`, { method: 'PATCH', body: JSON.stringify(payload) });
        toast.success('Category updated.');
      } else {
        await api('/admin/catalog/categories', { method: 'POST', body: JSON.stringify(payload) });
